...
########################################
# write total Gene Space for Jesse
# Genes in pathway commons - No CHEBI
foo <- Total[ !( grepl('CHEBI',Total$from)| grepl('CHEBI',Total$from) ), ]
val.genes <- c(foo$from,foo$to)
val.genes <- val.genes[!duplicated(val.genes)]

write.csv( val.genes, file = 'Non_CHEBI_Connected_Pathway_Commons_Genes.csv' )


parentId <- 'syn26844059'
activityName = 'All Pathway Commons Genes excluding CHEBI Links';
activityDescription = 'All Pathway Commons Genes excluding CHEBI Links';
...
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='01_Assemble_Network_Dataframe.Rmd', name = 'Genes In Pathway Commons', parentId=CODE$properties$id ), used = Syns_Used, executed = thisFile, activityName = activityName, activityDescription = activityDescription)

########################################

length(all.genes[!grepl('CHEBI',all.genes)])
# 19572
length(val.genes[!grepl('CHEBI',val.genes)])
# 19092
