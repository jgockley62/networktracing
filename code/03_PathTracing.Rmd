...
source('~/igraphNetworkExpansion/R/igraphplot2.R')
```
...
if( Filter_Edges == 'YES' ){
  #Net is: 15659 Vertices and 953,836 Edges
  # A)
  # loose( 372 vertices ) -- A == 15287 Vertacies and 647862 Edges
  ...
  # loose( 627 vertices ) -- C 15032 Vertacies and 509,168 Edges
  test_net <-  igraph::subgraph.edges( net,
                              igraph::E(net)[ ( igraph::E(net)$EdgeRep = 1  &
                                             igraph::E(net)$Occurance < 2 &
                                             igraph::E(net)$Avg_Cortex_CE == 0  ) == F ],
                              delete.vertices = TRUE
                            )

  # E)
  # loose( 3691 vertices ) -- A == 10926 Vertacies and 172,852 Edges
  test_net <-  igraph::subgraph.edges( net,
                              E(net)[ ( E(net)$EdgeRep == 1  & 
                                        E(net)$Avg_Cortex_CE == 0 ) == F ],
                              delete.vertices = TRUE
  ...
  # F)
  # loose(  vertices ) -- A == 15,287 Vertacies and 647,862 Edges
  test_net <-  igraph::subgraph.edges( net,
                              E(net)[ ( E(net)$SumOccurence < 2  & 
                                        E(net)$Avg_Cortex_CE == 0 ) == F ],
                              delete.vertices = TRUE
                            )
  # G)
  # loose(  vertices ) -- A == 10887 Vertacies and 142799 Edges
  test_net <- igraph::subgraph.edges( net,
                              E(net)[ ( (E(net)$EdgeRep == 1  | 
                                        E(net)$Occurance == 1) & 
                                        E(net)$Avg_Cortex_CE == 0 ) == F ],
  ...
  #H) only occurance
  # loose( 3691 vertices ) -- A == 10,887 Vertacies and 142,799 Edges
  test_net <-  igraph::subgraph.edges( net,
                              igraph::E(net)[ ( igraph::E(net)$Occurance < 2  & 
                                        igraph::E(net)$Avg_Cortex_CE == 0 ) == F ],
                              delete.vertices = TRUE
                            )
  #Use-F
...
OMICS_alt <- OMICS_alt[ !(OMICS_alt$ENSG == 'ENSG00000232995'),]
row.names( OMICS_alt ) <- OMICS_alt$GName

OMICS_dep <- OMICS_dep[ is.na(OMICS_dep$GName) == F, ]

keeps <- OMICS_dep[ duplicated(OMICS_dep$GName) & !(OMICS_dep$TYPE == 'None_Sig' ),]$GName
OMICS_dep <- OMICS_dep[ !(duplicated(OMICS_dep$GName) & (OMICS_dep$TYPE == 'None_Sig') & OMICS_dep$GName %in% keeps ),]
